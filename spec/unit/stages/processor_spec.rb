...
RSpec.describe Minigun::Stages::Processor do
  ...
  let(:task) do
    ...
    allow(task).to receive(:_minigun_hooks).and_return({})
  ...
  describe "#process" do
    ...
    it "processes the item with the processor block" do
      ...
      expect(task).to receive(:instance_exec) do |item, &block|
    ...
    context "when processing fails" do
      ...
      it "retries the specified number of times before failing" do
        ...
        expect {
          subject.process(5)
        }.to raise_error(RuntimeError, "Processing error")
      end
    end
  end
  ...
  describe "Processor without mocks" do
    ...
    describe "#process with real objects" do
      ...
      it "handles retries with real objects" do
        processor = Minigun::Stages::Processor.new(:failing_processor, real_pipeline, real_config)
        ...
        allow(processor).to receive(:emit) do |item, queue = :default|
        ...
        allow(real_task).to receive(:instance_exec) do |item, &block|
