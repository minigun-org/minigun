...
RSpec.describe Minigun::Task do
  ...
  describe 'connection options' do
    ...
    it 'processes queue subscriptions' do
      task.process_connection_options(:consumer, { queues: [:high_priority, :low_priority] })
      
      expect(task.queue_subscriptions[:consumer]).to eq([:high_priority, :low_priority])
    end
  end
  ...
  describe 'validation' do
    it 'validates COW consumer placement' do
      ...
      expect {
        ...
        task2.define_singleton_method(:validate_consumer_placement) do |consumer_type, name|
