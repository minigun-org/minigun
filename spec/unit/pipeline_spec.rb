...
RSpec.describe Minigun::Pipeline do
  ...
  describe "Pipeline without mocks" do
    ...
    it "builds a functional pipeline with real stages" do
      ...
      pipeline = Minigun::Pipeline.new(real_task, test_config)
      ...
      producer = pipeline.instance_variable_get(:@stages)[:test_producer][:instance]
      processor = pipeline.instance_variable_get(:@stages)[:test_processor][:instance]
      consumer = pipeline.instance_variable_get(:@stages)[:test_consumer][:instance]
      ...
      # Validate results
      expect(real_task.processor_output).to match_array([2, 4, 6])
      expect(real_task.consumer_input).to match_array([2, 4, 6])
    end
    ...
    it "connects custom stages correctly without mocks" do
      ...
      pipeline = Minigun::Pipeline.new(branching_task, test_config.merge(custom: true))
      ...
      # Validate results
      expect(branching_task.processor1_output).to match_array([2, 4, 6])
      expect(branching_task.processor2_output).to match_array([3, 6, 9])
    end
  end
end 