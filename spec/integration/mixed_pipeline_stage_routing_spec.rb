  ...
  describe 'pipeline to stage routing' do
    ...
    it '1-to-many: single pipeline routes to multiple stages' do
      test_class = Class.new do
        ...
        pipeline :source, to: %i[stage_a stage_b] do
          producer :gen do |output|
            2.times { |i| output << i }
          end
    ...
    it 'many-to-many: multiple pipelines route to multiple stages' do
      test_class = Class.new do
        ...
        pipeline :source_a, to: %i[stage_x stage_y] do
          producer :gen do |output|
            output << 1
          end
        ...
        pipeline :source_b, to: %i[stage_x stage_y] do
          producer :gen do |output|
            output << 2
          end
    ...
    it '1-to-many: single stage routes to multiple pipelines' do
      test_class = Class.new do
        ...
        pipeline do
          ...
          processor :split, to: %i[pipe_a pipe_b] do |item, output|
            output << (item * 10)
          end
        end
    ...
    it 'many-to-many: multiple stages route to multiple pipelines' do
      test_class = Class.new do
        ...
        pipeline do
          ...
          processor :stage_a, to: %i[pipe_x pipe_y] do |item, output|
            output << (item * 10)
          end
          ...
          processor :stage_b, to: %i[pipe_x pipe_y] do |item, output|
            output << (item * 20)
          end
        end
  ...
  describe 'pipeline from stage routing' do
    ...
    it '1-to-many: single pipeline receives from multiple stages' do
      test_class = Class.new do
        ...
        pipeline :receiver, from: %i[stage_a stage_b] do
    ...
    it 'many-to-many: multiple pipelines receive from multiple stages' do
      test_class = Class.new do
        ...
        pipeline :pipe_x, from: %i[stage_a stage_b] do
          processor :transform do |item, output|
            output << (item + 100)
          end
        ...
        pipeline :pipe_y, from: %i[stage_a stage_b] do
          processor :transform do |item, output|
            output << (item + 200)
          end
    ...
    it '1-to-many: single stage receives from multiple pipelines' do
      test_class = Class.new do
        ...
        pipeline do
          ...
          processor :merge, from: %i[source_a source_b] do |item, output|
            output << (item + 1000)
          end
    ...
    it 'many-to-many: multiple stages receive from multiple pipelines' do
      test_class = Class.new do
        ...
        pipeline do
          ...
          processor :stage_x, from: %i[source_a source_b] do |item, output|
            @mutex.synchronize { @results_x << (item + 100) }
            output << (item + 100)
          end

          processor :stage_y, from: %i[source_a source_b] do |item, output|
