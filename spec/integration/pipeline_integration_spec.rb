...
RSpec.describe 'Pipeline Integration' do
  describe 'running a pipeline without mocks' do
    ...
    it 'manually creates and runs a simple pipeline with direct method calls' do
      ...
      Minigun::Stages::Processor.new(:test_producer, pipeline,
                                     task.class._minigun_config.merge(stage_role: :producer))
      Minigun::Stages::Processor.new(:test_processor, pipeline, task.class._minigun_config)
      Minigun::Stages::Processor.new(:test_consumer, pipeline,
      ...
      expect(task.received_batches).to contain_exactly([2, 4], [6])
      expect(task.received_batches.flatten).to contain_exactly(2, 4, 6)
    end
  end
  ...
  describe 'running a pipeline with custom connections' do
    ...
    it 'manually creates and runs a branching pipeline with direct method calls' do
      ...
      expect(task.produced_items).to eq([1, 2, 3])
      expect(task.doubled_items).to contain_exactly(2, 4, 6)
      expect(task.tripled_items).to contain_exactly(3, 6, 9)
    end
  end
end
