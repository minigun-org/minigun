...
RSpec.describe 'Task Integration' do
  describe 'running without full pipeline mocks' do
    ...
    it 'simulates running the task without full pipeline execution' do
      ...
      expect(task.processed_items).to contain_exactly(1, 2, 3, 4, 5)
      expect(task.emitted_values).to contain_exactly(2, 4, 6, 8, 10)
      ...
      expect(task.consumed_batches).to contain_exactly([2, 4], [6, 8], [10])
      expect(task.consumed_batches.flatten).to contain_exactly(2, 4, 6, 8, 10)
    end
  end
