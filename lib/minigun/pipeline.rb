...
module Minigun
  ...
  class Pipeline
    ...
    def connect_stages
      ...
      connections = @task.class.respond_to?(:_minigun_connections) ? 
                    @task.class._minigun_connections : {}
      queue_subscriptions = @task.class.respond_to?(:_minigun_queue_subscriptions) ? 
      ...
      if queue_subscriptions && !queue_subscriptions.empty?
    ...
    def run
      ...
      producer_futures = []
      @stages.each do |stage_name, stage_info|
        if stage_info[:instance].respond_to?(:stage_role) && stage_info[:instance].stage_role == :producer
      ...
      @queues.each do |queue_name, _queue|
    ...
    def build_pipeline_from_task_definition
      ...
      if @task.class._minigun_pipeline_definition && @task.class._minigun_pipeline.empty?
        ...
        dsl_methods = [:producer, :processor, :accumulator, :consumer, :cow_fork, :ipc_fork]
        ...
        dsl_methods.each do |method_name|
          if @task.respond_to?(method_name)
            original_methods[method_name] = @task.method(method_name)
          end
        ...
        begin
          ...
          (dsl_methods + [:emit, :emit_to_queue]).each do |method_name|
            @task.singleton_class.remove_method(method_name)
            if original_methods[method_name]
              @task.define_singleton_method(method_name, original_methods[method_name])
            end
          end
        end
      end
      ...
      @task.class._minigun_pipeline.each do |stage_info|
        ...
        stage_class = stage_type == :accumulator ? 
      ...
      @stages.each do |stage_name, stage_info|
        ...
        if stage.is_a?(Stages::Processor) && 
          ...
          if @config[:custom]
            ...
            if stage_info[:input_connections] && stage_info[:input_connections].any?
    ...
    def shutdown_stages
      ...
      @stages.each do |stage_name, stage_info|
        if stage_info[:instance].respond_to?(:shutdown)
          begin
