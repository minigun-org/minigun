  ...
  class Worker
    ...
    def create_stage_context
      ...
      sources_expected = if @stage.run_mode == :autonomous
                           Set.new
                         elsif @stage.display_name == :_entrance && @pipeline.input_queues
    ...
    def log_debug(msg)
      ...
      Minigun.logger.debug "[Pipeline:#{@pipeline.name}][#{@stage.log_type}:#{display_name}] #{msg}"
    end

    def log_error(msg)
      display_name = @stage.display_name
      Minigun.logger.error "[Pipeline:#{@pipeline.name}][#{@stage.log_type}:#{display_name}] #{msg}"
    end
  end
end
