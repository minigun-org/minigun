...
module Minigun
  ...
  module Errors
    ...
    class StageNameConflict < ConfigurationError
      # @return [Symbol, String] The conflicting stage name
      attr_reader :stage_name

      # @return [String] The pipeline where the conflict occurred
      attr_reader :pipeline_name

      # @param stage_name [Symbol, String] The conflicting stage name (required)
      # @param pipeline_name [String] The pipeline name (required)
      def initialize(stage_name:, pipeline_name:)
        @stage_name = stage_name
        @pipeline_name = pipeline_name
        super(
          "Stage name '#{stage_name}' already exists in pipeline '#{pipeline_name}'",
          stage_name: stage_name,
          pipeline_name: pipeline_name
        )
      end
    ...
    class AmbiguousRouting < ConfigurationError
      # @return [Symbol, String] The ambiguous stage name
      attr_reader :stage_name

      # @return [Array<String>] List of candidate stage paths
      attr_reader :candidates

      # @param stage_name [Symbol, String] The ambiguous stage name (required)
      # @param candidates [Array<String>] List of matching stage paths (required)
      def initialize(stage_name:, candidates:)
        @stage_name = stage_name
        @candidates = candidates
        super(
          "Stage name '#{stage_name}' is ambiguous - found #{candidates.size} matches",
          stage_name: stage_name,
          candidates: candidates
        )
      end
    ...
    class InvalidOption < ConfigurationError
      # @return [Symbol, String] The option name
      ...
      # @param option_name [Symbol, String] The option name (required)
      # @param value [Object, nil] The invalid value (optional)
      # @param expected [String, nil] Description of expected values (optional)
      def initialize(option_name:, value: nil, expected: nil)
        @option_name = option_name
        @value = value
        @expected = expected
        super(
          "Invalid #{option_name}: #{value.inspect}. Expected: #{expected}",
          option_name: option_name,
          value: value,
          expected: expected
        )
    ...
    class CyclicDependency < PipelineError
      # @return [Symbol, String, Object] The source stage of the edge causing the cycle
      attr_reader :from_stage

      # @return [Symbol, String, Object] The target stage of the edge causing the cycle
      attr_reader :to_stage

      # @param from_stage [Symbol, String, Object] Source stage (required)
      # @param to_stage [Symbol, String, Object] Target stage (required)
      # @param pipeline_name [String, nil] The pipeline name (optional)
      def initialize(from_stage:, to_stage:, pipeline_name: nil)
        @from_stage = from_stage
        @to_stage = to_stage
        super(
          "Circular dependency detected: adding edge #{from_stage} -> #{to_stage} would create a cycle",
          pipeline_name: pipeline_name,
          from_stage: from_stage,
          to_stage: to_stage
        )
    ...
    class UnresolvedReference < PipelineError
      # @return [Symbol, String] The unresolved reference
      ...
      # @param reference [Symbol, String] The unresolved reference (required)
      # @param pipeline_name [String, nil] The pipeline name (optional)
      # @param available_stages [Array] List of available stages (optional)
      def initialize(message, reference:, pipeline_name: nil, available_stages: [])
        @reference = reference
        @available_stages = available_stages
        super(message, pipeline_name: pipeline_name, reference: reference)
      end
    end
    ...
    class SerializationFailed < PipelineError
      # @return [String] The class name of the item that couldn't be serialized
      attr_reader :item_class

      # @return [Exception, nil] The original serialization error
      attr_reader :original_error

      # @param item_class [String] The item's class name (required)
      # @param original_error [Exception, nil] The underlying error (optional)
      def initialize(item_class:, original_error: nil)
        @item_class = item_class
        @original_error = original_error
        super(
          "Cannot serialize item of type #{item_class}: #{original_error&.message}",
          item_class: item_class
        )
      end
    end
    ...
    class ItemProcessingFailed < StageError
      ...
      # @return [Exception] The original error that caused the failure
      attr_reader :original_error

      # @param stage_name [Symbol, String] The stage name (required)
      # @param original_error [Exception] The underlying error (required)
      # @param item [Object, nil] The item that failed (optional)
      def initialize(stage_name:, original_error:, item: nil)
        @item = item
        @original_error = original_error
        super(
          "Error processing item in stage '#{stage_name}': #{original_error.message}",
          stage_name: stage_name,
    ...
    class RetryExhausted < StageError
      ...
      # @return [Exception] The last error before giving up
      attr_reader :original_error

      # @param attempts [Integer] Number of attempts (required)
      # @param original_error [Exception] The last error (required)
      # @param stage_name [Symbol, String, nil] The stage name (optional)
      def initialize(attempts:, original_error:, stage_name: nil)
        @attempts = attempts
        @original_error = original_error
        super(
    ...
    class HookFailed < ExecutionError
      # @return [Symbol] The hook type (:before, :after, :before_fork, etc.)
      attr_reader :hook_type

      # @return [Symbol, String, nil] The stage name if this is a stage hook
      attr_reader :stage_name

      # @return [Exception] The original error from the hook
      attr_reader :original_error

      # @param hook_type [Symbol] The hook type (required)
      # @param original_error [Exception] The underlying error (required)
      # @param stage_name [Symbol, String, nil] The stage name (optional)
      def initialize(hook_type:, original_error:, stage_name: nil)
        @hook_type = hook_type
        @stage_name = stage_name
        @original_error = original_error
        stage_part = stage_name ? " for '#{stage_name}'" : ''
        super(
          "Hook #{hook_type}#{stage_part} failed: #{original_error.message}",
          hook_type: hook_type,
          stage_name: stage_name
        )
        set_backtrace(original_error.backtrace) if original_error.backtrace
    ...
    class CircuitBreakerOpen < ExecutionError
      # @return [Symbol, String] The circuit breaker name/identifier
      attr_reader :circuit_name

      # @return [Float, nil] Seconds until the circuit may close
      attr_reader :retry_after

      # @param circuit_name [Symbol, String] The circuit name (required)
      # @param retry_after [Float, nil] Seconds until retry (optional)
      def initialize(circuit_name:, retry_after: nil)
        @circuit_name = circuit_name
        @retry_after = retry_after
        super(
          "Circuit breaker '#{circuit_name}' is open. Retry after #{retry_after&.round(1)}s",
          circuit_name: circuit_name,
          retry_after: retry_after
        )
      end
    ...
    class ClusterConnectionFailed < ClusterError
      # @return [String] The URI that failed to connect
      attr_reader :uri

      # @return [Exception] The original connection error
      attr_reader :original_error

      # @param uri [String] The failed URI (required)
      # @param original_error [Exception] The underlying error (required)
      def initialize(uri:, original_error:)
        @uri = uri
        @original_error = original_error
        super(
          "Failed to connect to #{uri}: #{original_error.message}",
          uri: uri
        )
      end
    end
    ...
    class ClusterWorkerNotFound < ClusterError
      # @return [Symbol, String] The stage name that couldn't be found
      attr_reader :stage_name

      # @return [Array<Symbol, String>] List of stages available on workers
      attr_reader :available_stages

      # @param stage_name [Symbol, String] The missing stage name (required)
      # @param available_stages [Array] Available stages on workers (optional)
      def initialize(stage_name:, available_stages: [])
        @stage_name = stage_name
        @available_stages = available_stages
        super(
          "No worker has processor for stage '#{stage_name}'",
          stage_name: stage_name,
          available_stages: available_stages
        )
      end
    ...
    class ClusterDeliveryFailed < ClusterError
      # @return [Object] The item ID that failed delivery
      attr_reader :item_id

      # @return [Integer] Number of delivery attempts made
      attr_reader :attempts

      # @return [Exception, nil] The last error encountered
      attr_reader :last_error

      # @param item_id [Object] The item identifier (required)
      # @param attempts [Integer] Number of attempts (required)
      # @param last_error [Exception, nil] The last error (optional)
      def initialize(item_id:, attempts:, last_error: nil)
        @item_id = item_id
        @attempts = attempts
        @last_error = last_error
        super(
          "Failed to deliver item #{item_id} after #{attempts} attempts",
          item_id: item_id,
          attempts: attempts
        )
      end
    ...
    class ClusterTimedOut < ClusterError
      # @return [String] Description of the operation that timed out
      attr_reader :operation

      # @return [Float, Integer] The timeout duration in seconds
      attr_reader :timeout_seconds

      # @param operation [String] The operation description (required)
      # @param timeout_seconds [Float, Integer] The timeout value (required)
      def initialize(operation:, timeout_seconds:)
        @operation = operation
        @timeout_seconds = timeout_seconds
        super(
          "Cluster operation '#{operation}' timed out after #{timeout_seconds}s",
          operation: operation,
          timeout_seconds: timeout_seconds
        )
      end
