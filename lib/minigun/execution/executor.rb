  ...
  module Execution
    ...
    class ClusterPoolExecutor < Executor
      ...
      def execute_coordinator_mode(stage, input_queue, output_queue)
        ...
        Minigun.logger.info "[Cluster] Stage :#{stage.name} has #{@coordinator.worker_count} workers connected"
        ...
        if barrier && barrier.registered_count > 1
          debug_log("[Cluster:#{stage.name}] Waiting at barrier (#{barrier.ready_count + 1}/#{barrier.registered_count})")
          barrier.ready(stage.name)
          debug_log("[Cluster:#{stage.name}] Barrier released")
          # Small yield to allow other threads to progress after barrier
          Thread.pass
        end

        debug_log("[Cluster:#{stage.name}] Post-barrier, about to start distribute_and_collect")
        Minigun.logger.info "[Cluster] Starting stage :#{stage.name} distribution"
