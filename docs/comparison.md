...
Below are good-faith attempts to describe
If I got anything wrong, please raise a PR to improve this documentation.
or if Im missing your library, 

The point of this section is not to "convince" you to use Minigun, only 
everything is trade-offs, where you'll spend your time writing code.
...
produce(User.find_in_batches(5_000)) # Array<User>
cow_forks(8) do                       # 8 processes (COW on preloaded batch)
debatch                             # -> User
consume(threads: 10) do |user|      # 10 threads per child
...
Parallel is a quick tool for executing enumerator loops (`each`/`map`) with threads or processes.
It is perfect for simple usage scenarios, but it does not provide multi-stage flow
(produce → transform → consume) or back-pressure. In addition, Parallel's `in_processes` marshals
items in each child fork, while Minigun can use more efficient COW forking.
...
### Minigun vs. Async gem + async-container
...
abstraction like Minigun. The async-container library, originally written for the
Falcon webserver, supports a multi-process supervision tree, but is originally design for
running isolated workers in each process; it is not inherently suited for executing
pipeline flows *across processes*.
...
container.run(count: 8) do
  ...
  Async do |task|
    task.async do
      # You may partition users by id range per worker for less duplication
