...
module Publisher
  class PublisherBase
    ...
    DEFAULT_MAX_RETRIES = Rails.env.test? ? 1 : 10
    TIME_ZONE = 'Asia/Tokyo'
    LOCALE = :en
    MODEL_INCLUDES = {}.freeze
    MODELS_TRANSACTIONAL = %w[ Vesper::Table::Reservation
                               Vesper::Table::ReservationRequest
                               TableCheck::ReservationDraft
                               ...
                               Vesper::Table::SmsMessage
                               Vesper::Core::MailerLog
                               Vesper::Api::MessageEvent
                               Vesper::Table::PhoneCall
                               Vesper::Table::EdmDelivery
                               Vesper::Table::EdmSubscription
                               Vesper::Table::Payment
                               Vesper::Table::PaymentTransaction
                               Vesper::Table::PosJournal
                               Vesper::Table::SocialAuth
                               Vesper::Table::MembershipAuth
    ...
    def initialize(models: nil,
      ...
      @raw_franchise_ids = Array(franchise_ids).map(&:to_s).uniq if franchise_ids.present?
    ...
    def franchise_ids
      ...
      if @raw_franchise_ids.blank?
        return (@franchise_ids = nil)
      end

      @franchise_ids = @raw_franchise_ids.select {|id| BSON::ObjectId.legal?(id) }
      @franchise_ids |= Vesper::Table::Franchise.unscoped.any_in(_slugs: @raw_franchise_ids).pluck(:_id).map(&:to_s)
      @franchise_ids |= Vesper::Table::Shop.unscoped.any_of(
        [
          { _id: { '$in' => @raw_franchise_ids } },
          { _slugs: { '$in' => @raw_franchise_ids } }
        ]
      ).distinct(:franchise_id).map(&:to_s)
    end
      ...
      Thread.new do
        with_mongo_secondary(model) do
          ...
          time_range_in_batches(model).each do |range|
            ...
            on_failure = ->(e, _attempts) do
              Bugsnag.notify(e) {|r| r.add_metadata('publisher', model: model_name, time_range: format_time_range(range)) }
              Rails.logger.error { "[Producer] #{model_name}: Failed fetching IDs in #{format_time_range(range)} after #{max_retries} attempts: #{e.class}: #{e.message}. Skipping." }
            end
    ...
    def model_base_scope(model)
      ...
      if franchise_ids.present?
        case cached_model_relation(model)
        when :franchise
          scope = franchise_ids.size == 1 ? scope.where(_id: franchise_ids.first) : scope.any_in(_id: franchise_ids)
        when :belongs_to_franchise
          scope = franchise_ids.size == 1 ? scope.where(franchise_id: franchise_ids.first) : scope.any_in(franchise_id: franchise_ids)
        when :belongs_to_shop
          scope = franchise_shop_ids.size == 1 ? scope.where(shop_id: franchise_shop_ids.first) : scope.any_in(shop_id: franchise_shop_ids)
        end
      end
      scope
    end

    def franchise_shop_ids
      @franchise_shop_ids ||= Vesper::Table::Shop.unscoped.any_in(franchise_id: franchise_ids).pluck(:_id).map(&:to_s)
    end

    def cached_model_relation(model)
      @cached_model_relations ||= {}
      @cached_model_relations[model] ||= begin
        if model == Vesper::Table::Franchise
          :franchise
        elsif model.relations['franchise']&.class == Mongoid::Association::Referenced::BelongsTo
          :belongs_to_franchise
        elsif model.relations['shop']&.class == Mongoid::Association::Referenced::BelongsTo
          :belongs_to_shop
        else
          :other
        end
    ...
    def check_accumulator(accumulator_map)
      ...
      if (count = accumulator_map.values.sum(&:size)) > ACCUMULATOR_MAX_ALL_QUEUES
    ...
    def consume_object_ids(object_map)
      ...
      object_map.each do |model, object_ids|
        object_ids.uniq.each_slice(CONSUMER_THREAD_BATCH_SIZE) do |object_ids_batch|
    ...
    def start_consumer_thread(model, object_ids)
      ...
      Thread.new do
        with_mongo_secondary(model) do
          ...
          object_ids.each_slice(CONSUMER_QUERY_BATCH_SIZE) do |object_ids_batch|
            ...
            on_failure = ->(e, _attempts) do
              Bugsnag.notify(e) {|r| r.add_metadata('publisher', model: model_name, object_ids: object_ids_batch) }
              Rails.logger.error { "[Consumer]#{format_pid}, Thread #{thread_index}: Failed consuming #{model_name} after #{max_retries} attempts: #{e.class}: #{e.message}. Skipping." }
            end
            with_retry(on_retry: on_retry, on_failure: on_failure) do
    ...
    def time_range
      @time_range ||= in_time_zone_and_locale do
        ...
        raise ArgumentError.new('Must specify :start_time (can be "auto")') unless start_time
    ...
    def start_time_auto
      return unless @start_time&.to_s&.casecmp?('auto')
      model_base_scope(Vesper::Table::Franchise).min(:created_at)&.in_time_zone&.beginning_of_day
    end
    ...
    def time_range_batch_size(model)
      @time_range_batch_size ||= if franchise_ids
                                   model.to_s.in?(MODELS_TRANSACTIONAL) ? 1.day : 1.month
                                 else
                                   model.to_s.in?(MODELS_TRANSACTIONAL) ? 1.hour : 1.day
                                 end
    end
    ...
    def max_processes
      @max_processes ||= ENV['NUM_PROCESSES']&.to_i ||
    ...
    def sleep_with_backoff(attempts)
      sleep((2**attempts) / 100.0 + rand(0.05..1))
    end

    def with_mongo_secondary(model, &)
      read_opts = { mode: :secondary_preferred }
      node_type = use_analytics_node ? 'ANALYTICS' : 'ELECTABLE'
      read_opts[:tag_sets] = [{ 'nodeType' => node_type }] unless Settings.env.app_name == 'szuo'
    ...
    def job_info_data(finished: false)
      ...
      data[:analytics_db]   = use_analytics_node
      data.compact_blank!
    end
